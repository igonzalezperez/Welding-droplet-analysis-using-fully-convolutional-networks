...
def compute_properties(name, dataset):
    '''
    ...
    img = cv2.imread('HSV Frames\\Preds\\'+dataset+'\\Masks\\' +
                     str(name) + '.jpg', cv2.IMREAD_GRAYSCALE)
    ...
    # my_dpi = 96
    # img_shape = get_shape()
    # fig = plt.figure(figsize=(img_shape[0]/my_dpi, img_shape[1]/my_dpi),
...
def save_properties():
    ...
    area_arr = []
    perim_arr = []
    for i in progressbar.progressbar(range(len(os.listdir('HSV Frames/Preds/' +
                                                          DATASET + '/Masks/')))):

        img = Image.open('HSV Frames/Test/' +
                         DATASET + '/' + str(i) + '.jpg')

        img_mask = Image.open('HSV Frames/Preds/' +
                              DATASET + '/Masks/' + str(i) + '.jpg')

        cents, area, perimeter = compute_properties(i, DATASET)
        cents_arr.append(cents)
        area_arr.append(area)
        perim_arr.append(perimeter)

        # img.convert('L')
        # img = draw_centroid(cents, img)
        # img.convert('L')
        # img_mask = draw_centroid(cents, img_mask)
        # img_mask = img_mask.convert('L')
        # img.save('HSV Frames/Preds/' +
        #          dataset + '/Centroids/' + str(i) + '.jpg')

        # get_concat_h(img, img_mask).save(
        #     'HSV Frames/Preds/' +
        #     dataset + '/Inputs and Masks/' + str(i) + '.jpg')
    ...
    with open('Geometry/' + DATASET + '.pickle', 'wb') as f:
...
if __name__ == "__main__":
    img = Image.open('HSV Frames\\Test\\Spray\\2686.jpg').convert('L')
    plt.imshow(img)
    plt.show()

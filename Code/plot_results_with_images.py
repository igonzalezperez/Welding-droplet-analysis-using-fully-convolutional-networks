...
def plot_strip(num, save=False):
    ...
    Kwargs:
    save {bool} -- whether to save the plot as a .pgf or display it with plt.show().
    '''
    ...
    mask_strip[mask_strip > 0] = 255
    diff = np.diff(mask_strip)
    intersection_id = [np.where(diff == 255)[0][0]+1,
                       np.where(diff == -255)[0][0]]

    fig = plt.figure(figsize=set_size(aspect_ratio=1, subplots=(4, 4)))
